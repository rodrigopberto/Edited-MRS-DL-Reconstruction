...
class ONPipeline:
    ...
    def _pre_model_process_x(self,x_input,ppm):
        ...
        on_spec_norm = spec_norm[:,:,1,:]
        on_spec_norm = on_spec_norm.unsqueeze(3)
        on_spec_norm = torch.cat([torch.real(on_spec_norm),torch.imag(on_spec_norm)],axis=3)
        on_spec_norm = on_spec_norm.permute(0,3,1,2)

        return on_spec_norm,max_values
    ...
    def _post_model_process_reverse(self,target_on,max_value):
        #post_model_y = (data.target-min_value.unsqueeze(1))/(max_value.unsqueeze(1)-min_value.unsqueeze(1))
        post_model_y = (target_on/max_value.unsqueeze(1))
        return post_model_y
    ...
    def train_step(self,data,loss_fn,optim):
        ...
        y_model = self._post_model_process_reverse(data.target_on,max_value)
