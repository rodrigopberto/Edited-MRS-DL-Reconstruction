...
from mrs_acc.data.utils import max_min_gaba_normalization,max_naa_off_normalization
...
class DiffPipeline:
    ...
    def _pre_model_process_x(self,x_input,ppm):
        spec_norm,max_values = max_naa_off_normalization(x_input,ppm)
        diff_spec_norm = spec_norm[:,:,1,:]-spec_norm[:,:,0,:]
        diff_spec_norm = diff_spec_norm.unsqueeze(3)
        diff_spec_norm = torch.cat([torch.real(diff_spec_norm),torch.imag(diff_spec_norm)],axis=3)
