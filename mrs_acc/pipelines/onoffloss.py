...
class OnOffLossPipeline:
    ...
    def train_step(self,data,loss_fn,optim):
        ...
        y_model_diff = self._post_model_process_reverse(data.target,max_value)
        y_model_diff = y_model_diff.to(self.device)

        y_model_off = self._post_model_process_reverse(data.target_off,max_value)
        y_model_off = y_model_off.to(self.device)

        y_model_on = self._post_model_process_reverse(data.target_on,max_value)
        y_model_on = y_model_on.to(self.device)

        pred_diff,pred_off,pred_on = self.model(x_model,data.ppm[0])

        loss = loss_fn(pred_diff,pred_off,pred_on,data.ppm[0],y_model_diff,y_model_off,y_model_on)
    ...
    def val_step(self,data,loss_fn):
        ...
        with torch.no_grad():
            ...
            pred,_,_ = self.model(x_model,data.ppm[0])
    ...
    def predict(self,data):
        with torch.no_grad():
            ...
            x_model = x_model.to(self.device)
            y,_,_ = self.model(x_model,data.ppm[0])
