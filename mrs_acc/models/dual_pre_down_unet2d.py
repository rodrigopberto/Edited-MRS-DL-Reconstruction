...
class DualPreDownUnet2D(nn.Module):
    ...
    def __init__(self,initial_filters=16,transients=40,input_channels=2,kernel_size=3,
        ...
        super(DualPreDownUnet2D,self).__init__()
        ...
        self.combine_net = CNN1DSingleChain(depth=final_depth,input_filters=initial_filters*2,output_filters=initial_filters,kernel_size=kernel_size)
    ...
    def forward(self,x,ppm=None):
        ...
        y_off = self.off_net(x[:,::2])
        y_on = self.on_net(x[:,1::2])

        mid_y = torch.cat([y_off,y_on],axis=1)
