...
class RecurrentCNN(nn.Module):
    def __init__(self,steps=4,filters=16,transients=40,input_channels=2,kernel_size=5):

        super(RecurrentCNN,self).__init__()
        self.filters=filters

        self.cnn_1 = CNN(depth=steps,filters=filters,input_channels=input_channels,kernel_size=kernel_size)
        self.cnn_2 = CNN(depth=steps,filters=filters,input_channels=filters*2,kernel_size=kernel_size)
    
        self.final_conv = CNN(depth=steps,filters=filters,input_channels=filters,kernel_size=kernel_size)
        self.final_layer = nn.Conv1d(filters,1,kernel_size=1)
    ...
    def forward(self,x,ppm=None):
        ...
        hidden_input = torch.zeros(size=(x.shape[0],self.filters,x.shape[2])).to(og_device)
        for i in range(n_steps):
            y = self.cnn_1(x[:,:,:,i])
            y = torch.cat([hidden_input,y],axis=1)
            hidden_input = self.cnn_2(y)
        
        output = self.final_layer(F.tanh(self.final_conv(hidden_input))).squeeze(1)

        return output


class CNN(nn.Module):
    def __init__(self,depth=4,filters=16,input_channels=2,kernel_size=5):
        super(CNN,self).__init__()
        self.depth = depth
        in_channels = [input_channels]+[filters]*(depth-1)
        out_channels = [filters]*depth

        self.cnn = nn.ModuleList([nn.Conv1d(in_channels[i],out_channels[i],kernel_size=kernel_size,padding="same") for i in range(depth)])
        self.batchNorms = nn.ModuleList([nn.BatchNorm1d(out_channels[i]) for i in range(depth)])

    def forward(self,x):
        y=x
        for i in range(self.depth):
            y = F.tanh(self.batchNorms[i](self.cnn[i](y)))
        
        return y

class Recurrent1DUnet(nn.Module):

    def __init__(self,steps=4,zoom_filters=16,long_filters=8,transients=40,input_channels=4,kernel_size=5):
        ...
        self.pre_conv = nn.Sequential(nn.Conv1d(in_channels=input_channels,out_channels=zoom_filters,kernel_size=kernel_size,padding="same"),
                                       nn.BatchNorm1d(zoom_filters),nn.ReLU(inplace=True))
        
        self.large_unet_1d = Unet1D(steps=steps,initial_filters=long_filters,input_channels=input_channels*transients)

        self.final_layer = nn.Conv1d(in_channels=zoom_filters+long_filters,out_channels=1,kernel_size=kernel_size,padding="same")
    ...
    def forward(self,x,ppm=None):
        ...
        for step in range(steps):
            ...
            pre_x = self.pre_conv(zoom_x[:,:,:,step])
            ...
            y = self.decoder(hidden_input,output_list)
        ...
        output = self.final_layer(joint_out)
